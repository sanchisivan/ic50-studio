      ...
      mainPanel(
        ...
        tabsetPanel(
          ...
          tabPanel(
            ...
            tags$p("3. Set 'Uploaded concentration values' so the app knows whether your file stores linear concentrations or already log10-transformed values."),
            tags$p("4. Leave Curve direction on Auto-detect unless you have a strong reason to force Increasing or Decreasing."),
            tags$p("5. Choose a model, or turn on 'Compare all models first (no bootstrap)' to get a fast suggestion."),
            tags$p("6. Click Run analysis."),
            tags$p("7. For final reporting, turn on IC50 uncertainty only after you are happy with the model and plot."),
            br(),
            ...
            tags$p("For x-axis ticks, start with 'X-axis breaks = Automatic'. Switch to 'Measured concentrations' only if you want the tested doses shown explicitly, or use 'Custom' when you need exact user-defined ticks."),
            tags$p("If the x-axis is showing log10(concentration) labels, enter custom x breaks and x-axis limits in that same displayed log10 scale."),
            tags$p("Use 'Error bar cap width (% of x-axis span)' to control the visual cap width without tying it to the raw concentration spacing."),
            br(),
            h4("Data format"),
            ...
            tags$p("The Hill slope controls how sharply the response changes around the midpoint. In most assays it should be treated as a descriptive steepness term, not as a literal count of binding sites."),
            ...
            tags$p("For most enzyme inhibitor, viability, and standard dose-response datasets, start with 4PL and only move to 5PL if the asymmetry looks reproducible and the fit is more biologically realistic, not just numerically better."),
            br(),
